...
//
// Рачет идет по формуле: ((18 * СредняяСкоростьВКм/ч * 1.79) * ВесСпортсменаВКг / mInKM  * ВремяТренировкиВЧасах * minInH).
//
// Параметры:
//
// action int — количество совершенных действий(число шагов при ходьбе и беге, либо гребков при плавании).
...
//
// Рачет идет по формуле: ((0.035 * ВесСпортсменаВКг + (СредняяСкоростьВМетрахВСекунду**2 / РостВМетрах) * 0.029 * ВесСпортсменаВКг) * ВремяТренировкиВЧасах * minInH).
//
// Параметры:
//
// action int — количество совершенных действий(число шагов при ходьбе и беге, либо гребков при плавании).
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	...
	heightInM := float64(height) / float64(cmInM)
	meanSpeedMInS := meanSpeed(action, duration) * kmhInMsec // СредняяСкоростьВМетрахВСекунду

	return ((walkingCaloriesWeightMultiplier*weight + (math.Pow(meanSpeedMInS, 2.)/heightInM)*walkingSpeedHeightMultiplier*weight) * duration * minInH)
}
...
//
// Рачет идет по формуле:(СредняяСкоростьВКм/ч + 1.1) * 2 * ВесСпортсменаВКг * ВремяТренировкиВЧасах
//
// Параметры:
//
// lengthPool int — длина бассейна в метрах.
